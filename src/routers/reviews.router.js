...
router.post('/sitters/:sitterId/reviews', async (req, res, next) => {
  ...
  const userId = 3;  // 임시 유저 ID
  const currentDate = new Date();
  const date = new Date(currentDate.toISOString().split('T')[0]); // YYYY-MM-DD 형식으로 변환 후 다시 Date 객체로 변환

  if (!comment || !rating) {
    return res.status(400).json({ error: 'comment와 rating은 필수 입력사항입니다' });
  }

  try {
    // 펫시터가 존재하는지 확인
    const petSitter = await prisma.petSitter.findUnique({
      where: { id: parseInt(sitterId, 10) },
    });

    if (!petSitter) {
      return res.status(404).json({ error: '등록된 펫시터가 없습니다.' });
    }
    ...
    if (existingReview) {
      return res.status(400).json({ error: '이미 해당 날짜에 리뷰를 작성하셨습니다.' });
    }
...
router.get('/sitters/:sitterId/reviews/:reviewId', async (req, res, next) => {
  ...
  try {
    ...
    if (review.sitterId !== parseInt(sitterId, 10) || review.userId !== userId) {
      return res.status(403).json({ error: '해당 리뷰를 조회할 권한이 없습니다.' });
    }
...
router.patch('/sitters/:sitterId/reviews/:reviewId', async (req, res, next) => {
  ...
  if (!re_comment && !re_rating) {
    return res.status(400).json({ error: 're_comment 또는 re_rating 중 하나는 필수 입력사항입니다' });
  }
